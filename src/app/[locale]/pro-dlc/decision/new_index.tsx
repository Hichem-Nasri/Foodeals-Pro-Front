    ...
    <div className='flex w-full flex-col gap-4 pb-4 lg:pb-0'>
      ...
      <div className='hidden w-full justify-end rounded-[18px] bg-white p-2 lg:flex'>
        <DropDownList setSheet={setSheet} setImage={setImage}>
          <span>{t('dlc.decision.add', 'Add a DLC')}</span>
          <ShoppingBasket size={22} />
        </DropDownList>
      </div>
      <div className='flex w-full items-center justify-between px-2 lg:hidden'>
        <h1 className='text-xl font-medium text-lynch-900'>{t('dlc.decision.title', 'Our decisions')}</h1>
        <CustomButton
          label=''
          variant='ghost'
          size={'sm'}
      ...
      {data?.length === 0 && !isLoading ? (
        <EmptyListPlaceholder
          title={t('dlc.decision.noDecisions', 'No decisions')}
          description={t(
            'dlc.decision.noDecisionsDescription', 
            'You have not yet made decisions on products. Please add a decision for each product.'
          )}
          color='yellow'
        />
      ) : (
