    ...
    (
        {
            ...
            className,
            disabled = false,
            color = 'bg-mountain-400',
        },
        ref
    ) => {
        const t = useValuationTranslations()
        const sliderLabel = label || t('reduction')
                    ...
                    >
                        ...
                        {disabled ? (
                            ...
                            <Badge
                                variant='secondary'
                                className={cn(
                                    `bg-mountain-400 px-3 py-1.5 text-white`,
                                    color
                                )}
                            >
                                {value}%
                        ...
                        >
                            <SliderPrimitive.Track className='relative top-1/2 h-[3px] w-full grow -translate-y-1/2 rounded-full bg-lynch-200'>
                                <SliderPrimitive.Range
                                    className={cn(
                                        `absolute h-[6px] rounded-full bg-mountain-400`,
                                        color
                                    )}
                                />
                            ...
                            <SliderPrimitive.Thumb
                                className={cn(
                                    `block size-[22.5px] rounded-full border-white bg-mountain-400 transition-colors focus-within:border-[2.5px] focus-within:shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50`,
                                    color
                                )}
                            />
                        </SliderPrimitive.Root>
