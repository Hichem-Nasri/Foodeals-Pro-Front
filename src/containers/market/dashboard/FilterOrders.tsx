...
export const FilterOrders: FC<FilterOrdersProps> = ({
  form,
  onSubmit,
  setOpen,
  open,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger className='flex h-14 w-14 items-center justify-center gap-3 rounded-full bg-white p-3 text-lynch-500 hover:bg-transparent hover:text-black lg:h-fit lg:w-fit lg:rounded-[12px] lg:border lg:border-lynch-300 lg:bg-transparent lg:px-4 lg:text-lynch-500'>
        <span className='hidden font-medium lg:flex'>{disabled ? "..." : tCommon('filter').replace(' par', '')}</span>
        <ListFilter size={26} />
      </DialogTrigger>
      <DialogContent
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        <MobileHeader title={tCommon('filter')} onClick={() => setOpen(false)} />
        <DialogDescription className='flex flex-col gap-5 gap-x-4 px-4 py-5'>
          <div className='flex w-full flex-col gap-5 lg:flex-row'>
            ...
            <Select
              label={t('dashboard.filter.sortBy')}
              options={[t('dashboard.filter.new'), t('dashboard.filter.popular')].map((status) => ({
                key: status,
                label: capitalize(status) as string,
              }))}
              value={t('dashboard.filter.new')}
              onChange={(e) => {}}
            />
          </div>
          <div className='flex w-full flex-col gap-5 text-sm lg:flex-row'>
            <InputFieldForm
              control={control}
              name='codebar'
              label={t('dashboard.filter.barCode')}
              placeholder={t('dashboard.filter.barCodePlaceholder')}
              IconLeft={Mail}
            />
            <SelectField
              control={control}
              name='rayon'
              label={t('dashboard.filter.section')}
          ...
          <div className='flex w-full flex-col gap-5 lg:flex-row'>
            ...
            <InputFieldForm
              control={control}
              name='quantity'
              label={t('dashboard.filter.quantity')}
              placeholder={t('dashboard.filter.quantityPlaceholder')}
            />
          </div>
          <div className='flex w-full flex-col gap-5 lg:flex-row'>
            <SelectField
        ...
        <div className='sticky bottom-0 left-0 right-0 z-40 flex h-fit w-full items-center space-x-2 rounded-t-[24px] bg-white p-4 lg:relative lg:flex-auto lg:justify-end lg:bg-transparent'>
          <CustomButton
            variant='outline'
            label={t('dashboard.filter.cancel')}
            onClick={() => {
              setOpen(false)
            }}
          ...
          />
          <CustomButton
            label={t('dashboard.filter.confirm')}
            onClick={() => {}}
            className='h-fit w-full px-5 py-3 lg:w-fit'
