...
const DonateHistory: React.FC<indexProps> = () => {
  ...
  const status = (searchParams.get('status') as 'pending' | 'valid' | 'cancel') || 'pending'
  const currentPage = parseInt(searchParams.get('page') || '1', 10)
  
  // Function to update URL with new parameters
  const updateStatus = useCallback((newStatus: 'pending' | 'valid' | 'cancel') => {
  ...
  const [state, setState] = useState<'pending' | 'valid' | 'cancel'>(status)
  ...
  const { data, error, isLoading, isRefetching, refetch } = useQuery({
    queryKey: ['donation-orders', currentPage, totals.pageSize, status, archive],
    queryFn: async () => {
      try {
        const res = await api.get(`/donates/today/donations?status=${status}&page=${currentPage}&size=${totals.pageSize}`)
        if (res.status !== 200) {
