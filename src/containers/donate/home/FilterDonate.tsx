...
import { CircleCheckBig, ListFilter, Mail, X } from 'lucide-react'
...
interface FilterDonateProps {
  form: UseFormReturn<z.infer<typeof FilterSchemaProduct>>
  onSubmit: (data: z.infer<typeof FilterSchemaProduct>) => void
  setOpen: React.Dispatch<React.SetStateAction<boolean>>
  open: boolean
}

export const FilterDonate: FC<FilterDonateProps> = ({
  form,
  onSubmit,
  setOpen,
  open,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger className='flex h-14 w-14 items-center justify-center gap-3 rounded-full bg-white p-3 text-lynch-500 hover:bg-transparent hover:text-black lg:h-fit lg:w-fit lg:rounded-[12px] lg:border lg:border-lynch-300 lg:bg-transparent lg:px-4 lg:text-lynch-500'>
        <span className='hidden font-medium lg:flex'>{tDonate('actions.filter')}</span>
        <ListFilter size={26} />
      </DialogTrigger>
      <DialogContent
    ...
    <Form {...form}>
      ...
      >
        <DialogTitle className='mt-0 hidden items-center justify-between text-[1.375rem] font-normal text-lynch-400 lg:flex'>
          {tDonate('filter.filterBy')}
          <DialogClose>
            <X size={24} />
          </DialogClose>
        </DialogTitle>
        <MobileHeader title={tDonate('filter.filterBy')} onClick={() => setOpen(false)} />
        <DialogDescription className='flex flex-col gap-5 gap-x-4 px-4 py-5'>
          <div className='flex w-full flex-col gap-5 lg:flex-row'>
            <CustomSearch setSearch={setSearch} search={search} />
            <Select
              label={tDonate('filter.sortBy')}
              options={[tDonate('filter.new'), tDonate('filter.popular')].map((status) => ({
                key: status,
                label: capitalize(status) as string,
              }))}
              value={tDonate('filter.new')}
              onChange={(e) => {}}
            />
          </div>
          <div className='flex w-full flex-col gap-5 text-sm lg:flex-row'>
            <InputFieldForm
              control={control}
              name='codebar'
              label={tDonate('filter.barCode')}
              placeholder={tDonate('filter.enterBarCode')}
              IconLeft={Mail}
            />
            <SelectField
              control={control}
              name='rayon'
              label={tDonate('filter.section')}
              options={['Product', 'FOOD_BANK'].map((type) => ({
                key: type,
                label: capitalize(type as string) as string,
              }))}
            />
          </div>
          <div className='flex w-full flex-col gap-5 lg:flex-row'>
            ...
            <InputFieldForm
              control={control}
              name='quantity'
              label={tDonate('filter.quantity')}
              placeholder={tDonate('filter.quantityPlaceholder')}
            />
          </div>
          <div className='flex w-full flex-col gap-5 lg:flex-row'>
            <SelectField
              control={control}
              name='city'
              label={tDonate('filter.city')}
              options={[]}
            />
            <SelectField
              control={control}
              name='zone'
              label={tDonate('filter.zone')}
              options={[]}
            />
          </div>
        </DialogDescription>
